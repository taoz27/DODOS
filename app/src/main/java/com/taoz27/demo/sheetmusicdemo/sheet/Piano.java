...
public class Piano extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public static final int MaxOctave = 7;

    private static float WhiteKeyWidth;  /** Width of a single white key */
    private static float WhiteKeyHeight; /** Height of a single white key */
    private static float BlackKeyWidth;  /** Width of a single black key */
    private static float BlackKeyHeight; /** Height of a single black key */
    private static float margin;         /** The top/left margin to the piano (0) */
    private static float BlackBorder;    /** The width of the black border around the keys */

    private static float[] blackKeyOffsets;   /** The x pixles of the black keys */
    ...
    public static Point getPreferredSize(int screenwidth, int screenheight) {
        ...
        result.y = (int)(margin*2 + border*3 + WhiteKeyHeight);
        return result;
    }
    ...
    protected void onMeasure(int widthspec, int heightspec) {
        float screenwidth = MeasureSpec.getSize(widthspec);
        float screenheight = MeasureSpec.getSize(heightspec);
        WhiteKeyWidth = screenwidth / ( KeysPerOctave * MaxOctave+1);
...
//            WhiteKeyWidth--;
        ...
        float width = margin*2 + BlackBorder*2 + WhiteKeyWidth * KeysPerOctave * MaxOctave;
        float height = margin*2 + BlackBorder*3 + WhiteKeyHeight;
        setMeasuredDimension((int) width, (int) height);
        bufferBitmap = Bitmap.createBitmap((int) width, (int) height, Bitmap.Config.ARGB_8888);
    ...
    private void DrawOctaveOutline(Canvas canvas) {
        float right = WhiteKeyWidth * KeysPerOctave;
    ...
    private void DrawBlackBorder(Canvas canvas) {
        float PianoWidth = WhiteKeyWidth * KeysPerOctave * MaxOctave;
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(gray1);
    ...
    private void ShadeOneNote(Canvas canvas, int notenumber, int color) {
        ...
        float bottomHalfHeight = WhiteKeyHeight - (BlackKeyHeight+3);
