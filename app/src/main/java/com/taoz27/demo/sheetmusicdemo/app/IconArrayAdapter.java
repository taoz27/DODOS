...
public class IconArrayAdapter<T> extends ArrayAdapter<T> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
         ...
         MusicFile file = (MusicFile) this.getItem(position);
         if (file.isDirectory()) {
             image.setImageBitmap(directoryIcon);
             text.setText(file.getUri().getPath());
         }
         else {
             Bitmap album=file.getAlbum();
             image.setImageBitmap(album==null?midiIcon:album);
             text.setText((position+1)+". "+file.toString());
         }
         return convertView;
    }
}
