 ...
 */
public class MusicFile implements Comparator<MusicFile> {
    public enum TYPE{MP3,WAV,MID,LRC}

    private TYPE type;                /** The type to the file */
    private Uri uri;                  /** The URI path to the file */
    private String displayName;       /** The name to display */
    private Bitmap album;             /** The album of MP3 file to display */

    /** Create a Uri with the given display name */
    public MusicFile(TYPE type,Uri uri, String path) {
        this.album=null;
        this.type=type;
        this.uri = uri;
        if (path == null) {
    ...
    public static MusicFile fromJson(JSONObject obj, Activity activity) {
        try {
            TYPE type=TYPE.values()[obj.optInt("type",0)];
            ...
            return new MusicFile(type,uri, displayName);
        }
        catch (Exception e) {
            return null;
        }
